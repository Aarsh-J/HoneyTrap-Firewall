...
def get_client():
    ...
    if not hasattr(get_client, 'instance'):
        get_client.instance = HoneyTrapClient(config.SERVER_HOST, config.CONTROL_PORT, config.DATA_PORT, use_ssl=True)
...
def open_socket_user_portal(port, username="user"):
    ...
    from .gui.user_portal import open_user_portal
...
def open_socket_fake_portal(port):
    ...
    # Import within function to avoid circular imports
    from .gui.user_portal import open_fake_portal
